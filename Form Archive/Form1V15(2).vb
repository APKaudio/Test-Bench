...
Public Class UserInterface
    ...
    Dim ScopeGraph_Series_ch_1 As New Series
    Dim ScopeGraph_Series_ch_2 As New Series
    Dim ScopeGraph_Series_ch_3 As New Series
    Dim ScopeGraph_Series_ch_4 As New Series

    Dim Rigol_Scope_TimeBase As Double
    Dim Rigol_Scope_Scale As Double
    ...
    Private Sub Connecttoscope_Click(sender As Object, e As EventArgs) Handles Connecttoscope.Click
        Try
            RIGOL.IO = ioMgr.Open("TCPIP0::" & RigolIPAddres.Text & "::INSTR")    ' 10 is the instr. address 
            RIGOL.WriteString(":RUN")
            Connecttoscope.BackColor = Color.Red
        Catch ex As Exception
            MsgBox("Could no Connect to " & RigolIPAddres.Text & " Try again")
            Connecttoscope.BackColor = Color.Transparent
        End Try

        '     RIGOL.WriteString(":STOP")
    End Sub

    Public Sub Rigol_Read_Basic1()
        ...
        Rigol_dBm_1.Text = 10 * Math.Log10((CDbl(Rigol_Vrms_1.Text) ^ 2) / CDbl(dBm_ref_imp.Text) / (0.001))
    ...
    Public Sub Rigol_Read_Basic2()
        ...
        Rigol_dBm_2.Text = 10 * Math.Log10((CDbl(Rigol_Vrms_2.Text) ^ 2) / CDbl(dBm_ref_imp.Text) / (0.001))
        RIGOL.WriteString("MEASure:FREQuency? CHAN2")
        Rigol_Freq_2.Text = RIGOL.ReadString
    End Sub

    Public Sub Rigol_Read_Basic3()
        ...
        Rigol_dBm_3.Text = 10 * Math.Log10((CDbl(Rigol_Vrms_3.Text) ^ 2) / CDbl(dBm_ref_imp.Text) / (0.001))

        RIGOL.WriteString("MEASure:FREQuency? CHAN3")
        Rigol_Freq_3.Text = RIGOL.ReadString
    End Sub

    Public Sub Rigol_Read_Basic4()
        ...
        Rigol_dBm_4.Text = 10 * Math.Log10((CDbl(Rigol_Vrms_4.Text) ^ 2) / CDbl(dBm_ref_imp.Text) / (0.001))
    ...
    Private Sub Get_SCOPE_DATA_CH3_Click(sender As Object, e As EventArgs) Handles Get_SCOPE_DATA_CH3.Click
        RigolData_Fetch1()
        Rigol_Plot_Chart_ch_1()
        Rigol_Read_Basic1()

        RigolData_Fetch2()
        Rigol_Plot_Chart_ch_2()
        Rigol_Read_Basic2()

        RigolData_Fetch3()
        Rigol_Plot_Chart_ch_3()
        Rigol_Read_Basic3()

        RigolData_Fetch4()
        Rigol_Plot_Chart_ch_4()
        Rigol_Read_Basic4()

    End Sub

    Public Sub Rigol_Time_division()
        'get time scale
        RIGOL.WriteString(":TIMebase:MAIN:SCALe?")
        Rigol_Time_Base.Text = RIGOL.ReadString
        Rigol_Scope_TimeBase = Rigol_Time_Base.Text
        If Rigol_Scope_TimeBase < 0.000001 Then
            Rigol_Scope_Scale = 10000
            Rigol_Time_Scale.Text = "micro"
        ElseIf Rigol_Scope_TimeBase < 0.001 Then
            Rigol_Scope_Scale = 10
            Rigol_Time_Scale.Text = "Mili"
        Else
            Rigol_Scope_Scale = 1
            Rigol_Time_Scale.Text = "Other"
        End If
    End Sub


    Public Sub RigolData_Fetch1()
        ...
        Next I
        Rigol_Data_Grid_1.DataSource = SCOPE_CH_1
    End Sub

    Public Sub RigolData_Fetch2()
        ...
        Next I
        Rigol_Data_Grid_2.DataSource = SCOPE_CH_2
    End Sub
    Public Sub RigolData_Fetch3()
        ...
        Next I
        Rigol_Data_Grid_3.DataSource = SCOPE_CH_3
    End Sub
    Public Sub RigolData_Fetch4()
        ...
        Next I
        Rigol_Data_Grid_4.DataSource = SCOPE_CH_4
    End Sub
    ...
    Sub Rigol_Plot_Chart_ch_1()

        Rigol_Chart_1.Series.Clear()

        ScopeGraph_Series_ch_1.Points.Clear()
        ScopeGraph_Series_ch_1.Name = "Scope 1"

        'Change to a line graph.
        ScopeGraph_Series_ch_1.ChartType = SeriesChartType.Line
        ScopeGraph_Series_ch_1.Color = Color.DarkOrange
        Rigol_Time_division()
        For I = 0 To ((RigolSplitWaveData_CH1.Length) - 1)
            ScopeGraph_Series_ch_1.Points.AddXY(
                Math.Round((I * CDbl(Rigol_Time_Base.Text)) * Rigol_Scope_Scale, 3),
                RigolSplitWaveData_CH1(I))
        Next
        Rigol_Chart_1.Series.Add(ScopeGraph_Series_ch_1)
    End Sub
    Sub Rigol_Plot_Chart_ch_2()

        Rigol_Chart_2.Series.Clear()
        ScopeGraph_Series_ch_2.Points.Clear()

        ScopeGraph_Series_ch_2.Name = "SCOPE 3"

        'Change to a line graph.
        ScopeGraph_Series_ch_2.ChartType = SeriesChartType.Line
        ScopeGraph_Series_ch_2.Color = Color.LightBlue
        Rigol_Time_division()
        For I = 0 To ((RigolSplitWaveData_CH2.Length) - 1)
            ScopeGraph_Series_ch_2.Points.AddXY(
                Math.Round((I * CDbl(Rigol_Time_Base.Text)) * Rigol_Scope_Scale, 3),
                RigolSplitWaveData_CH2(I))
        Next
        Rigol_Chart_2.Series.Add(ScopeGraph_Series_ch_2)
    End Sub
    Sub Rigol_Plot_Chart_ch_3()
        Rigol_Chart_3.Series.Clear()
        ScopeGraph_Series_ch_3.Points.Clear()
        ScopeGraph_Series_ch_4.Name = "SCOPE 2"

        'Change to a line graph.
        ScopeGraph_Series_ch_3.ChartType = SeriesChartType.Line
        ScopeGraph_Series_ch_3.Color = Color.Violet
        Rigol_Time_division()
        For I = 0 To ((RigolSplitWaveData_CH3.Length) - 1)
            ScopeGraph_Series_ch_3.Points.AddXY(
                Math.Round((I * CDbl(Rigol_Time_Base.Text)) * Rigol_Scope_Scale, 3),
                RigolSplitWaveData_CH3(I))
        Next
        Rigol_Chart_3.Series.Add(ScopeGraph_Series_ch_3)
    End Sub
    Sub Rigol_Plot_Chart_ch_4()
        Rigol_Chart_4.Series.Clear()
        ScopeGraph_Series_ch_4.Points.Clear()
        ScopeGraph_Series_ch_4.Name = "Scope 3"

        'Change to a line graph.
        ScopeGraph_Series_ch_4.ChartType = SeriesChartType.Line
        ScopeGraph_Series_ch_4.Color = Color.DarkBlue
        Rigol_Time_division()
        For I = 0 To ((RigolSplitWaveData_CH4.Length) - 1)
            ScopeGraph_Series_ch_4.Points.AddXY(
                Math.Round((I * CDbl(Rigol_Time_Base.Text)) * Rigol_Scope_Scale, 3),
                RigolSplitWaveData_CH4(I))
        Next
        Rigol_Chart_4.Series.Add(ScopeGraph_Series_ch_4)
    End Sub

    Private Sub Button11_Click(sender As Object, e As EventArgs) Handles Rigol_update_CH1.Click
        Rigol_Read_Basic1()
        RigolData_Fetch1()
        Rigol_Plot_Chart_ch_1()
    End Sub

    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Rigol_update_CH2.Click
        Rigol_Read_Basic2()
        RigolData_Fetch2()
        Rigol_Plot_Chart_ch_2()
    End Sub

    Private Sub Button9_Click(sender As Object, e As EventArgs) Handles Rigol_update_CH3.Click
        Rigol_Read_Basic3()
        RigolData_Fetch3()
        Rigol_Plot_Chart_ch_3()
    End Sub

    Private Sub Button10_Click(sender As Object, e As EventArgs) Handles Rigol_update_CH4.Click
        Rigol_Read_Basic4()
        RigolData_Fetch4()
        Rigol_Plot_Chart_ch_4()
    End Sub
    ...
    Private Sub Button1_Click_3(sender As Object, e As EventArgs) Handles Button1.Click
        RIGOL.WriteString(":AUToscale")
    End Sub

    Private Sub DMM_dbm_2_TextChanged(sender As Object, e As EventArgs) Handles DMM_dbm_2.TextChanged

    End Sub

    Private Sub Rigol_Chart_1_Click(sender As Object, e As EventArgs) Handles Rigol_Chart_1.Click

    End Sub

    Private Sub Rigol_Data_Grid_1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles Rigol_Data_Grid_1.CellContentClick
    ...
    ''******************** PLOT THE DATA OF THE GRAPH *******************
    Private Sub Rigol_FFT_Chart_Click(sender As Object, ByVal e As MouseEventArgs) Handles Rigol_FFT_Chart.MouseMove
        ...
        Try
            ...
            RIGOL_FFT_TOOLTIP.SetToolTip(Rigol_FFT_Chart, "x:" & (dp.XValue) & "  A:" & dp.YValues(0))

            Dim Rigol_FFT_x_Cursor As Double = dp.XValue
            Dim Rigol_FFT_y_Cursor As Double = dp.YValue
